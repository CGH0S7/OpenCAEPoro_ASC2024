...
OCP_INT ScalarPetscSolver::Solve() { OCP_ABORT("Inavailable!"); }
...
OCP_INT VectorPetscSolver::Solve() {
  ...
  // old_petsc: msp
  // return FIM_solver_p(myrank, numproc, blockdim, allBegin.data(),
  // allEnd.data(), iA.data(), jA.data(), A.data(), b, x);

  // new_petsc: msp、cpr、bamg
  // return FIM_solver_p(precondID, is_thermal, myrank, numproc, blockdim,
  // allBegin.data(), allEnd.data(), iA.data(), jA.data(), A.data(), b, x,
  // &MATRIX_CONVERSION_TIME);
  int iter = FIM_solver_p(precondID, is_thermal, myrank, numproc, blockdim,
                          allBegin.data(), allEnd.data(), iA.data(), jA.data(),
                          A.data(), b, x, &MATRIX_CONVERSION_TIME);
