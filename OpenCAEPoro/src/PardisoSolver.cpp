...
void PardisoSolver::InitParam() {
  ...
  iparm[12] = 1; /* Switch on Maximum Weighted Matching algorithm (default for
                    non-symmetric) */
  ...
  iparm[39] =
      2; /* Input: matrix/rhs/solution are distributed between MPI processes  */
...
void PardisoSolver::AssembleMat(const vector<vector<USI>> &colId,
                                ...
                                const OCP_USI &dim, vector<OCP_DBL> &rhs,
                                vector<OCP_DBL> &u) {
...
OCP_INT PardisoSolver::Solve() {
  ...
  phase = 11;
  cluster_sparse_solver(pt, &maxfct, &mnum, &mtype, &phase, &N, A.data(),
                        iA.data(), jA.data(), &idum, &nrhs, iparm, &msglvl,
                        &ddum, &ddum, &myComm, &error);

  if (error != 0)
    OCP_ABORT("ERROR during symbolic factorization: " +
              to_string((long long int)error));
  ...
  if (error != 0)
    OCP_ABORT("ERROR during numerical factorization: " +
              to_string((long long int)error));
  ...
  phase = 33;
  cluster_sparse_solver(pt, &maxfct, &mnum, &mtype, &phase, &N, A.data(),
                        iA.data(), jA.data(), &idum, &nrhs, iparm, &msglvl, b,
                        x, &myComm, &error);

  if (error != 0)
    OCP_ABORT("ERROR during numerical solution: " +
              to_string((long long int)error));
  ...
  cluster_sparse_solver(pt, &maxfct, &mnum, &mtype, &phase, &N, &ddum,
                        iA.data(), jA.data(), &idum, &nrhs, iparm, &msglvl,
                        &ddum, &ddum, &myComm, &error);

  if (error != 0)
    OCP_ABORT("ERROR during release memory: " +
              to_string((long long int)error));
...
/// Calculate terms used in communication
void VectorPardisoSolver::CalCommTerm(const USI &actWellNum,
                                      const Domain *domain) {
  global_index = domain->CalGlobalIndex(actWellNum);
  ...
  iparm[40] =
      (global_end - numElementLoc + 1) * blockdim; // global begin (included)
      ...
      for (USI j1 = 0; j1 < nnzR; j1++) {
        if (global_index->at(colId[i - 1][j1]) == tmp[j0]) {
          ...
          for (USI c = 0; c < blockdim; c++) {
            ...
            const OCP_DBL *begin =
                &val[i - 1][0] + j1 * blockSize + c * blockdim;
