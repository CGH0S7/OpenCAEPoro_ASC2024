 ...
 *  \date    Feb/28/2023
 *
 *  \note    The params used in OpenCAEPoroX is mostly compatible with Eclipse
 *by SLB, but it has some own rules for easy to use. It is extensible and
 *friendly.
 *
 *-----------------------------------------------------------------------------------
...
protected:
  ...
  /// Well's global index(start from zero), perfortions' index(start from zero)
  /// and location(bulks' local index) of perforation
...
public:
  ...
  mutable vector<OCP_ULL>
      global_index; ///< Interior grid + active well + ghost grid in equations
