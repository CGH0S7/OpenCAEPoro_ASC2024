...
ControlTimeParam::ControlTimeParam(const TuningPair &src,
                                   const vector<OCP_DBL> &Tstep, const USI &i) {
...
void ControlTime::CutDt(const OCP_DBL &fac) {
  ...
  if (CURRENT_RANK == MASTER_PROCESS) {
    cout << "### WARNING: Cut time step size: " << fixed << setprecision(3)
         << ldt << TIMEUNIT + " -> " << setprecision(3) << current_dt
         << TIMEUNIT + "!\n";
  }
}
...
void ControlTime::SetNextTSTEP(const USI &i, const AllWells &wells) {
  ...
  MPI_Allreduce(&wellChange_loc, &wellOptChange, 1, OCPMPI_BOOL, MPI_LAND,
                myComm);
  ...
  if (dt <= 0)
    OCP_ABORT("Non-positive time stepsize!");
...
void ControlTime::CalNextTimeStep(const OCPNRsuite &NRs,
  ...
  for (auto &s : il) {
    if (s == "dP") {
      if (dPmax > TINY)
        factor = min(factor, wp->dPlim / dPmax);
    } else if (s == "dT") {
      // no input now -- no value
      if (dTmax > TINY)
        factor = min(factor, wp->dTlim / dTmax);
    } else if (s == "dN") {
      if (dNmax > TINY)
        factor = min(factor, wp->dNlim / dNmax);
    } else if (s == "dS") {
      if (dSmax > TINY)
        factor = min(factor, wp->dSlim / dSmax);
    } else if (s == "eV") {
      if (eVmax > TINY)
        factor = min(factor, wp->eVlim / eVmax);
    } else if (s == "iter") {
      if (NRs.GetIterNR() < 5)
  ...
  if (dt_loc > wp->timeMax)
    dt_loc = wp->timeMax;
  if (dt_loc < wp->timeMin)
    dt_loc = wp->timeMin;
...
OCPNRStateC ControlNR::CheckConverge(const OCPNRsuite &NRs,
                                     const initializer_list<string> &il) const {
  OCPNRStateC conflag_loc = OCPNRStateC::not_converge;
  for (auto &s : il) {
    if (s == "res") {
      if ((NRs.res.maxRelRes_V <= NRs.res.maxRelRes0_V * wp->tol ||
           NRs.res.maxRelRes_V <= wp->tol || NRs.res.maxRelRes_N <= wp->tol) &&
      ...
      if (fabs(NRs.DPBmaxNRc()) <= wp->dPmin &&
          fabs(NRs.DSmaxNRc()) <= wp->dSmin) {
        conflag_loc = OCPNRStateC::converge;
      }
    } else if (s == "dT") {
      if (fabs(NRs.DPBmaxNRc()) <= wp->dPmin &&
